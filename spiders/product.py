...
class Product(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        product['sub_title'] = normalize_str(
            r.css('div#content div.c-product__config > span::text').get())
        product['description'] = normalize_str(
            r.css('div#desc div > div::text').get())
        attribute_keys = r.css(
            'div#params li > div.c-params__list-key').getall()
        attribute_values = r.css(
            'div#params li > div.c-params__list-value').getall()
        ...
        for index, (attr_key, attr_val) in enumerate(attributes_tmp):
            ...
            val = normalize_str(
                Selector(text=attr_val).css('div > span::text').get())
            if key is None:
                attributes[-1]['val'] += '\n' + val
            else:
